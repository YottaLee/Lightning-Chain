...
func (n *Node) ForwardTransaction(ctx context.Context, in *pro.TransactionWithAddress) (*pro.Empty, error) {
	...
	txWithCount, ok := n.SeenTransactions[myTx.Hash()]
	// if already exist, then increase the count
	if ok {
		txWithCount.Count++
		n.SeenTransactions[myTx.Hash()] = txWithCount
	} else {
		// TODO how to verify
		// if not exit, then check

		sender := n.PeerDb.Get(addr)
		wit, err := sender.Addr.GetWitnessesRPC(in.Transaction)
		if err != nil {
			return nil, err
		}
		if myTx.Segwit {
			wit.GetWitnesses()
			myTx.Witnesses = append(myTx.Witnesses, wit.GetWitnesses()...)
...
func (n *Node) GetWitnesses(ctx context.Context, in *pro.Transaction) (*pro.Witnesses, error) {
	...
	txWithCount, ok := n.SeenTransactions[tx.Hash()]
	if !ok {
		return nil, nil
	}
	witness := txWithCount.Transaction.Witnesses
	return &pro.Witnesses{Witnesses: witness}, nil
}
