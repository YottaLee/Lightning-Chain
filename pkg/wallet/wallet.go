...
func (w *Wallet) HandleRevokedOutput(hash string, txo *block.TransactionOutput,
	...
	// TODO what is the unlocking script ?
	newTxi := &block.TransactionInput{
		ReferenceTransactionHash: hash,
	...
	newTxo := &block.TransactionOutput{
	...
	tx := &block.Transaction{
		Segwit:    false,
		Version:   0,
		Inputs:    []*block.TransactionInput{newTxi},
		Outputs:   []*block.TransactionOutput{newTxo},
		Witnesses: [][]byte{},
		LockTime:  0,
	}

	sig, _ := utils.Sign(w.Id.GetPrivateKey(), []byte(tx.Hash()))
	tx.Witnesses = [][]byte{sig}
	go func() {
		w.TransactionRequests <- tx
	}()
	w.Balance += newTxo.Amount
	return tx
}
...
func (w *Wallet) GenerateFundingTransaction(amount uint32, fee uint32, counterparty []byte) *block.Transaction {
	...
	outputs := w.generateFundingTransactionOutputs(amount, fee, counterparty, change)
	tx := &block.Transaction{
		Segwit:    true,
		Version:   0,
		Inputs:    inputs,
	...
	// TODO  do we need to update the balance ?
	return tx
}

func (w *Wallet) generateFundingTransactionOutputs(amount uint32, fee uint32, counterparty []byte, change uint32) []*block.TransactionOutput {
	if counterparty == nil || len(counterparty) == 0 {
		utils.Debug.Printf("[generateFundingTransactionOutputs] Error: counterparty's public key is invalid")
		return nil
	}
	// the outputs that we will eventually return
	var outputs []*block.TransactionOutput
	// the output for the person we're sending this transaction output to
	myScript := &pro.MultiParty{
	...
	if err != nil {
		myScriptB = []byte{}
		fmt.Printf("[wallet.generateFundingTransactionOutputs] Failed to marshal script")
	}
	txo1 := &block.TransactionOutput{
		Amount:        amount,
		LockingScript: myScriptB,
	}
	txo2 := &block.TransactionOutput{
		Amount:        0,
		LockingScript: myScriptB,
	}
	...
	outputs = append(outputs, txo1, txo2, txo3)
	return outputs
}
